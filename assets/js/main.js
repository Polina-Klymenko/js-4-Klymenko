...
let wordsList = function (str, substr) {
  ...
  arr.forEach(function (item) {
    if (item.indexOf(substr) !== -1) set.add(item);
  })
...
let getLocalDate = function (date, isSeconds, isISO) {
  ...
  (date.getMonth() < 10) ? mmo = ('0' + (date.getMonth() + 1)) : mmo = (date.getMonth() + 1);
  yyyy = date.getFullYear();
  (date.getHours() < 10) ? hh = ('0' + date.getHours()) : hh = date.getHours();
  (date.getMinutes() < 10) ? mmi = ('0' + date.getMinutes()) : mmi = date.getMinutes();
  (date.getSeconds() < 10) ? ss = ('0' + date.getSeconds()) : ss = date.getSeconds();

  if (!isISO) {
    !isSeconds && (strRes = `${dd}.${mmo}.${yyyy}, ${hh}:${mmi}`);
    isSeconds && (strRes = `${dd}.${mmo}.${yyyy}, ${hh}:${mmi}:${ss}`);
  }
  else {
    !isSeconds && (strRes = `${yyyy}-${mmo}-${dd}, ${hh}:${mmi}`);
    isSeconds && (strRes = `${yyyy}-${mmo}-${dd}, ${hh}:${mmi}:${ss}`);
  }
...
let getLocalDay = function (date) {
  date = new Date(date);
  (date.getDay() === 0) ? day = 7 : day = date.getDay();
